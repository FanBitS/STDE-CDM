...
def Solar_sce_gen(Solar_no, samples_no, seed=0):
    ...
    Solar_forecast = np.load(os.path.join(os.getcwd(), 'data', 'solar_forecast_prototype.npy'))
    Solar_forecast = Solar_forecast.reshape([1, -1, 1])
    Solar_forecast = np.tile(Solar_forecast, (1, 1, Solar_no))

    Solar_error_samples = np.load(os.path.join(os.getcwd(), 'data', 'solar_temporal_correlated_error_samples.npy'))
    Solar_error_samples = Solar_error_samples[:samples_no * Solar_no].reshape(samples_no, Solar_no, -1)
    ...
    print(f"  error scale factor (forecast std): {forecast_std:.4f}")
    ...
    if not np.allclose(Solar_samples_full, Solar_forecast + Solar_error_samples):
        raise ValueError('PV scenarios do not equal forecast plus error!')

    if np.min(Solar_samples_full) < 0:
        raise ValueError('PV scenarios contain negative values!')

    print(f"PV scenario generation done:")
...
def plot_paper_figures(save_path='figure'):
    ...
    Solar_no = 5
    samples_no = 200
    Solar_forecast, Solar_error_samples, Solar_samples_full = Solar_sce_gen(Solar_no, samples_no)
    ...
    ax1.hist(error_flat, bins=80, density=True, alpha=0.6,
             color='steelblue', edgecolor='white', label='Empirical distribution')
    ...
    ax1.set_xlabel('Normalized forecast error', fontsize=14, fontweight='bold')
    ax1.set_ylabel('Probability density', fontsize=14, fontweight='bold')
    ax1.set_title('PV Forecast Error Marginal Distribution', fontsize=16, fontweight='bold')
    ax1.legend(prop={'weight': 'bold', 'size': 13})
    ...
    plt.tight_layout()
    fig1.savefig(os.path.join(save_path, 'solar_error_distribution.pdf'), dpi=300, bbox_inches='tight')
    print(f"  saved: {save_path}/solar_error_distribution.pdf")
    plt.close(fig1)
    ...
    ax2.fill_between(hours, p5, p95, alpha=0.3, color='gold', label='90% confidence interval')
    ...
    ax2.set_xlabel('Hour', fontsize=14, fontweight='bold')
    ax2.set_ylabel('Normalized PV generation', fontsize=14, fontweight='bold')
    ax2.set_title('PV Generation Scenarios (24-hour dispatch horizon)', fontsize=16, fontweight='bold')
    ax2.set_xlim([0, T-1])
    ax2.set_ylim([0, 0.6])
    ax2.set_xticks(np.arange(0, T, 2))
    ax2.legend(prop={'weight': 'bold', 'size': 13}, loc='upper left')
    ...
    plt.tight_layout()
    fig2.savefig(os.path.join(save_path, 'solar_scenarios_24h.pdf'), dpi=300, bbox_inches='tight')
    print(f"  saved: {save_path}/solar_scenarios_24h.pdf")
    plt.close(fig2)
    ...
    sns.heatmap(temp_corr, mask=mask, cmap='coolwarm', center=0,
                square=True, linewidths=0.5, cbar_kws={"shrink": 0.8},
                annot=False, ax=ax3, vmin=-1, vmax=1)

    ax3.set_xlabel('Hour', fontsize=14, fontweight='bold')
    ax3.set_ylabel('Hour', fontsize=14, fontweight='bold')
    ax3.set_title('Temporal Correlation of PV Forecast Error', fontsize=16, fontweight='bold')
    ...
    plt.tight_layout()
    fig3.savefig(os.path.join(save_path, 'solar_temporal_correlation.pdf'), dpi=300, bbox_inches='tight')
    print(f"  saved: {save_path}/solar_temporal_correlation.pdf")
    plt.close(fig3)

    print("\n=== Figures generated ===")
    print(f"Figure save path: {os.path.abspath(save_path)}/")
